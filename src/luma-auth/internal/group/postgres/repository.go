...
func (r *Repository) Create(ctx context.Context, name string) (*group.Group, error) {
	...
	var g group.Group
	err := r.db.QueryRow(ctx, q, name).Scan(&g.ID, &g.Name, &g.CreatedAt, &g.UpdatedAt)
	if err != nil {
...
func (r *Repository) Rename(ctx context.Context, id, name string) (*group.Group, error) {
	const q = `
		...
		RETURNING id, name, created_at, updated_at`
	var g group.Group
	err := r.db.QueryRow(ctx, q, id, name).Scan(&g.ID, &g.Name, &g.CreatedAt, &g.UpdatedAt)
...
func (r *Repository) Get(ctx context.Context, id string) (*group.GroupWithDetails, error) {
	...
	err := r.db.QueryRow(ctx, q, id).Scan(&g.ID, &g.Name, &g.CreatedAt, &g.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, fmt.Errorf("group not found")
	}
	if err != nil {
...
func (r *Repository) List(ctx context.Context) ([]*group.GroupWithDetails, error) {
	const q = `SELECT id, name, created_at, updated_at FROM auth.groups ORDER BY name`
